// ============================================================
// 蓝牙RFCOMM文件传输接收方 - 实现
// ============================================================

#include "bluetooth/bt_transfer_receiver.h"
#include "bluetooth/bt_utils.h"
#include "common/protocol.h"
#include "common/utils.h"
#include <iostream>
#include <cstring>
#include <algorithm>
#include <fcntl.h>
#include <unistd.h>
#include <sys/stat.h>
#include <bluetooth/bluetooth.h>
#include <bluetooth/rfcomm.h>

constexpr int LISTEN_BACKLOG = 5;

// ============================================================
// 构造与析构
// ============================================================

BtTransferReceiver::BtTransferReceiver(uint8_t channel)
    : m_channel(channel)
    , m_save_dir("./received_files")
    , m_listen_socket(INVALID_SOCKET_FD)
    , m_running(false)
{
}

BtTransferReceiver::~BtTransferReceiver() {
    stop();
}

// ============================================================
// 公共接口
// ============================================================

bool BtTransferReceiver::start() {
    if (m_running.load()) return true;

    if (!create_listen_socket()) return false;

    m_running = true;
    m_accept_thread = std::thread(&BtTransferReceiver::accept_loop, this);

    std::cout << "[蓝牙接收] 传输接收服务启动 (通道 " << (int)m_channel << ")" << std::endl;
    std::cout << "[蓝牙接收] 保存目录: " << m_save_dir << std::endl;
    return true;
}

void BtTransferReceiver::stop() {
    if (!m_running.load()) return;

    std::cout << "[蓝牙接收] 正在停止传输接收服务..." << std::endl;
    m_running = false;

    if (m_listen_socket != INVALID_SOCKET_FD) {
        CLOSE_SOCKET(m_listen_socket);
        m_listen_socket = INVALID_SOCKET_FD;
    }

    if (m_accept_thread.joinable()) {
        m_accept_thread.join();
    }

    // 等待所有处理线程完成
    {
        std::lock_guard<std::mutex> lock(m_handler_mutex);
        for (auto& t : m_handler_threads) {
            if (t.joinable()) t.join();
        }
        m_handler_threads.clear();
    }

    std::cout << "[蓝牙接收] 传输接收服务已停止" << std::endl;
}

void BtTransferReceiver::set_on_receive_complete(ReceiveCompleteCallback callback) {
    m_complete_cb = std::move(callback);
}

void BtTransferReceiver::set_on_receive_start(ReceiveStartCallback callback) {
    m_start_cb = std::move(callback);
}

// ============================================================
// 创建监听 Socket
// ============================================================

bool BtTransferReceiver::create_listen_socket() {
    m_listen_socket = BtUtils::create_rfcomm_server(m_channel, LISTEN_BACKLOG);
    return m_listen_socket != INVALID_SOCKET_FD;
}

// ============================================================
// 接受连接循环
// ============================================================

void BtTransferReceiver::accept_loop() {
    std::cout << "[蓝牙接收] 接受连接线程启动" << std::endl;

    while (m_running.load()) {
        fd_set read_fds;
        FD_ZERO(&read_fds);
        FD_SET(m_listen_socket, &read_fds);

        struct timeval tv;
        tv.tv_sec = 0;
        tv.tv_usec = 200000;  // 200ms

        int sel = select((int)(m_listen_socket + 1),
                         &read_fds, nullptr, nullptr, &tv);
        if (sel < 0) break;
        if (sel == 0) continue;

        struct sockaddr_rc client_addr;
        socklen_t addr_len = sizeof(client_addr);
        SOCKET_FD client_sock = accept(m_listen_socket,
                                        (struct sockaddr*)&client_addr,
                                        &addr_len);
        if (client_sock == INVALID_SOCKET_FD) break;

        std::string sender_addr = BtUtils::bdaddr_to_str(client_addr.rc_bdaddr);

        std::cout << "[蓝牙接收] 发送方已连接: " << sender_addr << std::endl;

        // 创建处理线程
        std::thread handler([this, client_sock, sender_addr]() {
            handle_receive(client_sock, sender_addr);
        });

        // 管理处理线程
        {
            std::lock_guard<std::mutex> lock(m_handler_mutex);
            m_handler_threads.erase(
                std::remove_if(m_handler_threads.begin(), m_handler_threads.end(),
                    [](std::thread& t) { return !t.joinable(); }),
                m_handler_threads.end()
            );
            m_handler_threads.push_back(std::move(handler));
        }
    }

    std::cout << "[蓝牙接收] 接受连接线程退出" << std::endl;
}

// ============================================================
// 处理单个接收任务
// ============================================================

void BtTransferReceiver::handle_receive(SOCKET_FD client_sock,
                                         const std::string& sender_addr) {
    // 1. 接收文件头
    FileMeta meta;
    if (!recv_file_header(client_sock, meta)) {
        std::cerr << "[蓝牙接收] 接收文件头失败" << std::endl;
        CLOSE_SOCKET(client_sock);
        return;
    }

    std::cout << "[蓝牙接收] 接收文件: " << meta.filename
              << " (" << Utils::format_file_size(meta.file_size) << ")"
              << ", 分片: " << meta.total_chunks << std::endl;

    // 通知上层开始接收 (创建 TransferTask)
    if (m_start_cb) {
        m_start_cb(meta.file_id, meta.filename, meta.file_size,
                   meta.total_chunks, sender_addr);
    }

    // 2. 初始化文件 I/O (与 TCP 版本完全相同的逻辑)
    std::string save_path = m_save_dir + "/" + meta.filename;
    std::string temp_path = save_path + ".tmp";

    // 确保临时文件存在且大小正确
    int fd = open(temp_path.c_str(), O_WRONLY | O_CREAT, 0644);
    if (fd < 0) {
        std::cerr << "[蓝牙接收] 无法创建临时文件: " << temp_path << std::endl;
        CLOSE_SOCKET(client_sock);
        return;
    }

    // 获取当前临时文件大小
    struct stat st;
    if (fstat(fd, &st) == 0) {
        if (static_cast<uint64_t>(st.st_size) < meta.file_size) {
            (void) ftruncate(fd, static_cast<off_t>(meta.file_size));
        }
    }
    close(fd);

    // 创建 FileChunkIO (临时文件已存在且有正确大小)
    FileChunkIO io(save_path, meta.chunk_size, false);

    if (io.get_total_chunks() == 0) {
        std::cerr << "[蓝牙接收] 文件初始化失败: total_chunks=0" << std::endl;
        CLOSE_SOCKET(client_sock);
        return;
    }

    // 3. 接收分片循环
    uint32_t last_ack_count = 0;
    uint32_t chunk_count = 0;

    while (chunk_count < meta.total_chunks) {
        Chunk chunk;
        if (!recv_chunk(client_sock, chunk)) {
            std::cerr << "[蓝牙接收] 接收分片失败, 已接收: "
                      << chunk_count << std::endl;
            CLOSE_SOCKET(client_sock);
            return;
        }

        // 处理控制消息 (JSON, 非分片)
        if (chunk.chunk_index == 0xFFFFFFFF) {
            break;
        }

        // 写入分片到文件
        if (!io.write_chunk(chunk)) {
            std::cerr << "[蓝牙接收] 写入分片失败: chunk="
                      << chunk.chunk_index << std::endl;
            CLOSE_SOCKET(client_sock);
            return;
        }

        ++chunk_count;

        // 每收到1个分片就发送 ACK
        if (chunk_count > last_ack_count) {
            uint32_t max_cont = io.get_max_contiguous_chunk();
            send_ack(client_sock, meta.file_id, max_cont);
            last_ack_count = chunk_count;
        }
    }

    // 4. 发送最终 ACK
    uint32_t final_contiguous = io.get_max_contiguous_chunk();
    send_ack(client_sock, meta.file_id, final_contiguous);

    // 5. 完成传输, 验证文件
    if (final_contiguous == meta.total_chunks - 1) {
        if (io.commit_received_file()) {
            // 验证 MD5
            std::string actual_checksum = Utils::md5_file(save_path);
            if (actual_checksum == meta.checksum || meta.checksum.empty()) {
                std::cout << "[蓝牙接收] 文件接收完成, 校验通过: "
                          << meta.filename << std::endl;
                if (m_complete_cb) {
                    m_complete_cb(meta.file_id, save_path, true);
                }
            } else {
                std::cerr << "[蓝牙接收] MD5 校验失败! 预期: " << meta.checksum
                          << " 实际: " << actual_checksum << std::endl;
                if (m_complete_cb) {
                    m_complete_cb(meta.file_id, save_path, false);
                }
            }
        }
    } else {
        std::cerr << "[蓝牙接收] 文件不完整: " << final_contiguous + 1
                  << "/" << meta.total_chunks << std::endl;
        if (m_complete_cb) {
            m_complete_cb(meta.file_id, save_path, false);
        }
    }

    CLOSE_SOCKET(client_sock);
}

// ============================================================
// 接收文件头 (JSON + 'J' 标记)
// ============================================================

bool BtTransferReceiver::recv_file_header(SOCKET_FD sock, FileMeta& meta) {
    char marker;
    if (SOCK_RECV(sock, &marker, 1, 0) != 1 || marker != 'J') {
        std::cerr << "[蓝牙接收] 文件头类型标记错误: " << (int)marker << std::endl;
        return false;
    }

    json header;
    if (!Protocol::recv_json_message(sock, header)) {
        return false;
    }

    std::string type = header.value("type", "");
    if (type != "FILE_HEADER") {
        std::cerr << "[蓝牙接收] 预期 FILE_HEADER, 收到: " << type << std::endl;
        return false;
    }

    meta.file_id      = header.value("file_id", "");
    meta.filename     = header.value("filename", "");
    meta.file_size    = header.value("file_size", uint64_t(0));
    meta.checksum     = header.value("checksum", "");
    meta.chunk_size   = header.value("chunk_size", uint32_t(0));
    meta.total_chunks = header.value("total_chunks", uint32_t(0));

    std::cout << "[蓝牙接收] 文件头: " << meta.filename
              << " size=" << meta.file_size
              << " chunks=" << meta.total_chunks << std::endl;

    return true;
}

// ============================================================
// 接收单个分片 (二进制 + 'C' 标记)
// ============================================================

bool BtTransferReceiver::recv_chunk(SOCKET_FD sock, Chunk& chunk) {
    // 读取类型标记
    char marker;
    if (SOCK_RECV(sock, &marker, 1, 0) != 1) {
        return false;
    }
    if (marker == 'J') {
        // JSON 控制消息 (TRANSFER_DONE/ERROR 等)
        chunk.chunk_index  = 0xFFFFFFFF;
        chunk.total_chunks = 0xFFFFFFFF;
        return true;
    }
    if (marker != 'C') {
        std::cerr << "[蓝牙接收] 分片类型标记错误: " << (int)marker << std::endl;
        return false;
    }

    // 接收分片头部 (16 字节)
    uint8_t header[CHUNK_HEADER_SIZE];
    size_t total_read = 0;
    while (total_read < CHUNK_HEADER_SIZE) {
        auto n = SOCK_RECV(sock, reinterpret_cast<char*>(header) + total_read,
                           static_cast<int>(CHUNK_HEADER_SIZE - total_read), 0);
        if (n <= 0) return false;
        total_read += n;
    }

    // 解析头部获取 data_size 和 file_id_len
    uint32_t data_size   = ((uint32_t)header[8]  << 24) |
                           ((uint32_t)header[9]  << 16) |
                           ((uint32_t)header[10] << 8)  |
                            (uint32_t)header[11];
    uint32_t file_id_len = ((uint32_t)header[12] << 24) |
                           ((uint32_t)header[13] << 16) |
                           ((uint32_t)header[14] << 8)  |
                            (uint32_t)header[15];

    // 安全检查
    if (data_size > MAX_CHUNK_DATA_SIZE || file_id_len > 256) {
        std::cerr << "[蓝牙接收] 分片头部异常: data_size=" << data_size
                  << " file_id_len=" << file_id_len << std::endl;
        return false;
    }

    // 读取分片载荷 (file_id + data)
    size_t remaining = file_id_len + data_size;
    std::vector<uint8_t> body(remaining);

    total_read = 0;
    while (total_read < remaining) {
        auto n = SOCK_RECV(sock, reinterpret_cast<char*>(body.data()) + total_read,
                           static_cast<int>(remaining - total_read), 0);
        if (n <= 0) return false;
        total_read += n;
    }

    // 构造完整 buffer 并反序列化
    std::vector<uint8_t> full_buffer(CHUNK_HEADER_SIZE + remaining);
    memcpy(full_buffer.data(), header, CHUNK_HEADER_SIZE);
    memcpy(full_buffer.data() + CHUNK_HEADER_SIZE, body.data(), remaining);

    return chunk.deserialize(full_buffer.data(), full_buffer.size());
}

// ============================================================
// 发送 ACK (JSON + 'J' 标记)
// ============================================================

bool BtTransferReceiver::send_ack(SOCKET_FD sock, const std::string& file_id,
                                   uint32_t max_contiguous_chunk) {
    char marker = 'J';
    if (SOCK_SEND(sock, &marker, 1, 0) != 1) return false;

    json ack = Protocol::build_chunk_ack(file_id, max_contiguous_chunk);
    return Protocol::send_json_message(sock, ack);
}
